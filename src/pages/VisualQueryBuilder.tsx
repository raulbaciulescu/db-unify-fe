  ...
  const generateMockData = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      selectedColumns.forEach(col => {
        if (col.aggregate) {
          row[`${col.aggregate}(${col.column})`] = Math.floor(Math.random() * 1000);
        } else {
          row[`${col.table}.${col.column}`] = `Value ${i + 1}`;
        }
      });
      data.push(row);
    }
                  ...
                  >
                    ...
                    {selectedTables.map(table => (
                      <optgroup key={table.alias} label={`${table.name} (${table.alias})`}>
                        <option value={`${table.alias}.id`}>id</option>
                        <option value={`${table.alias}.name`}>name</option>
                        <option value={`${table.alias}.created_at`}>created_at</option>
                      </optgroup>
                    ))}
                  </select>
                  <button
